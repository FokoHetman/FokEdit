...
use crate::foklang::core::error_handler::ErrorHandler;
use crate::foklang::core::tokenizer::{Operator};
use std::{convert::TryInto};
use crate::foklang::core::env::Environment;
use crate::foklang::core::{tokenizer, parser, builtins};
...
impl Interpreter {
  fn evaluate_program(&mut self, prog: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    ...
    match prog.kind {
      AST::NodeKind::Program{body,id} => {
        ...
        for i in body {
          if counter==env.current_node {
            last_eval = self.evaluate(*i, env, program.clone());
            env.current_node+=1;
          }
          counter+=1;
        }
      ...
      _ => panic!("[Interpreter Error] Tried to evaluate non-Program Node as a Program, {:#?}", prog)
    }
    return last_eval
  }
  pub fn evaluate(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::Program{body:_,id:_} => self.evaluate_program(node, env, program),
      ...
      AST::NodeKind::BinaryExpression{left:_,right:_,operator:_} => self.evaluate_binary_expression(node, env, program),
      AST::NodeKind::Identifier{symbol:_, ..} => self.evaluate_identifier(node, env, program),
      AST::NodeKind::AdvancedDeclaration{..} => self.evaluate_list(node, env, program),
      AST::NodeKind::List{..} => self.evaluate_list(node, env, program),
      AST::NodeKind::ListConcat{..} => self.evaluate_list(node, env, program),
      AST::NodeKind::Config{..} => self.evaluate_object(node, env, program),
      AST::NodeKind::Access{..} => self.evaluate_object(node, env, program),
      AST::NodeKind::FunctionDeclaration{identifier: _, statement: _} => self.evaluate_function(node, env, program),
      AST::NodeKind::TypeDeclaration{identifier: _, ftype: _} => self.evaluate_function(node, env, program),
      AST::NodeKind::IfStatement{..} => self.evaluate_if(node,env, program),
      AST::NodeKind::Char{..} => self.evaluate_char(node, env, program),
      AST::NodeKind::Bool{..} => self.evaluate_bool(node, env, program),
      AST::NodeKind::Case{..} => self.evaluate_matchcase(node, env, program),
      AST::NodeKind::Match{..} => self.evaluate_matchcase(node, env, program),
      _ => panic!("{} {:#?}", self.error_handler.interpreter("unknown_node").error_msg, node)
    }
  }
  fn evaluate_matchcase(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::Case{value} => {
        self.evaluate(*value, env, program)
      }
      AST::NodeKind::Match{left, values} => {
        let mut t_env = Environment {parent: Some(Box::new(env.clone())), ..Default::default()};
        //println!("{:#?}", env);
        t_env.declare(AST::Node{kind: AST::NodeKind::Identifier{symbol: String::from("x"), childs: vec![]}}, self.evaluate(*left, env, program.clone()));
        
        let mut result = AST::Proventus{value: AST::Fructa::Nullus, id: -1};
        for i in values {
          let eval = self.evaluate(*i, &mut t_env, program.clone());
  ...
  fn evaluate_if(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::IfStatement{condition, body} => {
        match self.evaluate(*condition, env, program.clone()).value {
          AST::Fructa::Condicio(b) => {
            if b {
              return self.evaluate(*body.clone(), env, program.clone());
            };
          ...
          _ => self.evaluate(*body.clone(), env, program)
        }
      }
      _ => panic!("help")
    }
  }
  fn evaluate_bool(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_char(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
  ...
  fn nest(&mut self, bounds: Vec<(AST::Node, Vec<AST::Proventus>)>, body: Box<AST::Node>, nenv: &mut Environment, program: Program) -> Vec<AST::Proventus> {
    if bounds.len()==0 {
      vec![self.evaluate(*body.clone(), nenv, program)]
    } else {
      ...
      for x in bound.1 {
        ...
        result.append(&mut self.nest(bounds.clone(), body.clone(), &mut t2_env, program.clone()));
      }
      result
    }
  }
  fn evaluate_list(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::AdvancedDeclaration{body, assumptions} => {
        ...
        for i in assumptions {
          match i.kind {
            AST::NodeKind::FunctionDeclaration{identifier, statement} => {
              t_env.declare(*identifier.clone(), self.evaluate(*statement.clone(), env, program.clone()));
            },
                    ...
                    match y.kind {
                      AST::NodeKind::BinaryExpression{left: identifier2, right: bound, ..} => {
                        if *identifier2.clone() == *left.clone() {
                          ibounds.push(self.evaluate(*bound.clone(), env, program.clone()));
                        }
                    .clone() }
                      _ => panic!("23454")
                    }
              ...
              for i in list {
                ...
                for x in &checks {
                  ...
                  if self.evaluatable(x.clone(), &mut t2_env) {
                    //println!("{:#?}", x.clone());
                    let eval = self.evaluate(x.clone(), &mut t2_env, program.clone());
        ...
        let finale: Vec<AST::Proventus> = self.nest(final_bounds, body, &mut t_env, program);
      ...
      AST::NodeKind::ListConcat{item, list} => {
        let evaluated = self.evaluate(*list.clone(), env, program.clone());
        let evaluated_item = self.evaluate(*item.clone(), env, program);
      ...
      AST::NodeKind::List{body} => {
        ...
        for i in body {
          if self.evaluate(*i.clone(), env, program.clone()).id==112 {
            match self.evaluate(*i.clone(), env, program.clone()).value {
            ...
            if args.len()>0 {
              if args[0].value.display_type() == self.evaluate(*i.clone(), env, program.clone()).value.display_type() { // TODO:    make the comparision better
                args.push(self.evaluate(*i.clone(), env, program.clone()));
              } else {
                panic!("List of type `{}` supplied with `{}`", args[0].value.display_type(), self.evaluate(*i.clone(), env, program.clone()).value.display_type());
              }
            } else {
              args.push(self.evaluate(*i.clone(), env, program.clone()));
            }
  ...
  fn evaluate_function(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
        ...
        match (*statement).kind {
          AST::NodeKind::Identifier{ref childs, ..} => {
            ...
            if self.evaluatable(*statement.clone(), env) {
              let eval = self.evaluate(*statement, env, program);
  ...
  fn evaluate_binary_expression(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::BinaryExpression{left: node_left,right: node_right,operator: node_operator} => {
        ...
        match node_operator {
          Operator::Addition => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
          ...
          Operator::Substraction => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Numerum(i-i2), id: -1}
          ...
          Operator::Multiplication => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Numerum(i*i2), id: -1}
          ...
          Operator::Division => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
          ...
          Operator::DivideRest => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  if i2==0 {//error handle it i beg
          ...
          Operator::Exponentiation => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
          ...
          Operator::Comparision => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
          ...
          Operator::Greater => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Condicio(i>i2), id: -1}
          ...
          Operator::Lower => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Condicio(i<i2), id: -1}
          ...
          Operator::GreaterEqual => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Condicio(i>=i2), id: -1}
          ...
          Operator::LowerEqual => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
                  AST::Proventus{value: AST::Fructa::Condicio(i<=i2), id: -1}
          ...
          Operator::DoubleDot => match self.evaluate(*node_left, env, program.clone()).value {
            AST::Fructa::Numerum(i) => {
              match self.evaluate(*node_right, env, program).value {
                AST::Fructa::Numerum(i2) => {
          ...
          Operator::RightFatArrow => {
            let evaluated = self.evaluate(*node_left, env, program.clone());
            if match evaluated.value {
              ...
              self.evaluate(*node_right, env, program)
            } else {
              AST::Proventus{value: AST::Fructa::Nullus, id:-1}
            }
          },
  ...
  fn evaluate_identifier(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
            ...
            AST::NodeKind::Identifier{ref symbol, ref childs} => {
              ...
              for i in 0..childs.len() {
                ...
                match args[i].clone().kind {
                  AST::NodeKind::List{body} => {
                    let eval = self.evaluate(*childs[i].clone(), env, program.clone());
                    match eval.value {
                      AST::Fructa::Inventarii(_) => {
                        if self.evaluate(args[i].clone(), env, program.clone()) != eval {
                          continue 'ma
                        }
                      }
                  ...
                  AST::NodeKind::ListConcat{item, list} => {
                    match self.evaluate(*childs[i].clone(), env, program.clone()).value {
                  ...
                  AST::NodeKind::NumericLiteral{value} => {
                    ...
                    let rag = match self.evaluate(*childs[i].clone(), env, program.clone()).value {
              ...
              for i in 0..childs.clone().len() {
                ...
                match args[i].clone().kind {
                  AST::NodeKind::Identifier{..} => {one_arg_env.declare(args[i].clone(), self.evaluate(*childs[i].clone(), env, program.clone()));},
                  AST::NodeKind::NumericLiteral{value,..} =>  {},
                  AST::NodeKind::ListConcat{item, list} => {
                    match self.evaluate(*childs[i].clone(), env, program.clone()).value {
              ...
              if match final_function.value {
                ...
                result = self.evaluate(match final_function.value {AST::Fructa::Moenus(args, statement) => statement, _ => panic!(">>>")}, env, program.clone());
              } else {
      ...
      AST::Fructa::BuiltIn(f, cached_args) => {
        ...
        if f == builtins::print {
          ...
          for i in args_vec.clone() {
            //parse types!!
            n_args.push(self.evaluate(i, env, program.clone()));
          }
          ...
          if args_vec.len() >= 2 {
            fargs = builtins::FunctionArgs::double(self.evaluate(args_vec[0].clone(), env, program.clone()), 
            ...
            fargs = builtins::FunctionArgs::single(self.evaluate(args_vec[0].clone(), env, program.clone()));
          }
          expected = 2;
         ...
         if args_vec.len() >= 2 {
            fargs = builtins::FunctionArgs::fmap(self.evaluate(args_vec[0].clone(), env, program.clone()), self.evaluate(args_vec[1].clone(), env, program.clone()), env.clone(), self.clone(), program.clone());
          ...
          if args_vec.len()>=2 {
            fargs = builtins::FunctionArgs::double(self.evaluate(args_vec[0].clone(), env, program.clone()), self.evaluate(args_vec[1].clone(), env, program.clone()));
          } else if args_vec.len()==1 {
            fargs = builtins::FunctionArgs::single(self.evaluate(args_vec[0].clone(), env, program.clone()));
          }
          expected = 2;
          ...
          if args_vec.len()>=3 {
            fargs = builtins::FunctionArgs::triple(self.evaluate(args_vec[0].clone(), env, program.clone()), self.evaluate(args_vec[1].clone(), env, program.clone()), self.evaluate(args_vec[2].clone(), env, program.clone()));
          ...
          expected = 0;
        } else if f == builtins::read_file || f == builtins::envf || f == builtins::exec {
          if args_vec.len()>=1 {
            fargs = builtins::FunctionArgs::single(self.evaluate(args_vec[0].clone(), env, program.clone()))
          }
          expected = 1
          ...
          if args_vec.len()>=1 {
            fargs = builtins::FunctionArgs::load_file(self.evaluate(args_vec[0].clone(), env, program.clone()), env.clone(), self.clone(), program.clone());
          }
          expected = 1;
        }

        else if f == builtins::quit {
  ...
  fn evaluate_object(&mut self, node: AST::Node, env: &mut Environment, program: Program) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::Access{parent, value} => {
        match value.kind {
          AST::NodeKind::Identifier{symbol: vsymbol, ..} => {
            let eval = self.evaluate(*parent.clone(), env, program);
      ...
      AST::NodeKind::Config{arguments, flags} => {
        ...
        if flags.contains(&AST::ConfigFlag::CodeBlock) {
            ...
            match i.0.clone().kind {
              AST::NodeKind::Identifier{symbol,..} => {
                if symbol == String::from("FOKO_EVALUATE_NODE_I_WILL_KRILL") {
                  latest = self.evaluate(*i.1.clone(), &mut t_env, program.clone());
                } else {
                  latest = self.evaluate(AST::Node{kind: AST::NodeKind::FunctionDeclaration{identifier: Box::new(*i.0.clone()), statement: Box::new(*i.1.clone())}}, &mut t_env, program.clone());
                }
              }
              _ => {}
            }
          ...
          for i in arguments {
            ...
            args.push((*i.0.clone(), self.evaluate(*i.1.clone(), env, program.clone())));
          }
